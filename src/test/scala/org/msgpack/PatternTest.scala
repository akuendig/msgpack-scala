  ...
  */
  "Indexing" should {
    "index correctly" in {
      ...
      // confirm field order
      val data = write(o)
      val mirror = read[IndexingMirror](data)
  ...
  "Options" should {
    "throw error if name is null" in {
      val o = new Options
      o.name = null
      write(o) must throwA[MessageTypeException]
    }
    "ignore @ignore" in {
      val o = new Options
  ...
  "CaseClass" should {
    "throw error if the generic type has Trait bound and thus implicit constructor parameters" in {
      write(new CaseClassTypeClass[Float]()) must throwA[MessageTypeException]
      write(CaseClassTypeClass[Float]()) must throwA[MessageTypeException]
      write(CaseClassTypeClass(42)) must throwA[MessageTypeException]
    }
    "throw error if it has a generic type parameter and is not previously registered" in {
      write(new GenericCaseClass[String]()) must throwA[MessageTypeException]
      write(GenericCaseClass[String]()) must throwA[MessageTypeException]
      write(GenericCaseClass("Dumbu")) must throwA[MessageTypeException]
    }
    "encode / decode without generic parameter" in {
      checkOn(new CaseClass(), "value")
      checkOn(CaseClass(), "value")
      checkOn(CaseClass(42), "value")
